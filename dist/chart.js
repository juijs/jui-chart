    ...
    var ZoomScrollWidget = function(chart, axis, widget) {
        ...
        this.draw = function() {
            ...
            return chart.svg.group({}, function() {
                ...
                l_rect = chart.svg.pathRect(areaStyle);
                l_rect.round(lw, h, radius, 0, 0, radius);

                r_rect = chart.svg.pathRect(areaStyle);
                r_rect.round(rw, h, 0, radius, radius, 0);
                r_rect.translate(w - rw, 0);

                c_rect = chart.svg.rect({
                    width: w - lw - rw,
                    height: h,
                    fill: "transparent",
                    "fill-opacity": 0,
                    stroke: chart.color(widget.color),
                    "stroke-width": chart.theme("zoomScrollAreaBorderWidth")
                }).translate(lw, 0);

                var l_img = chart.svg.image({
                    x: lw - size/2,
                    y: h / 2 - size/2,
                    width: size,
                    height: size,
